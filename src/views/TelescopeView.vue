...
const telescopeModules = computed(() =>
  modules.value
    .filter((m) => 'ITelescope' in m.interfaces)
...
const currentModule = computed(() =>
  routeJid.value ? telescopeModules.value.find((m) => m.jid === routeJid.value) : undefined,
)

// No :jid in the URL: redirect to the first online module, same convention
// as RoofView.vue.
watchEffect(() => {
  if (!routeJid.value && telescopeModules.value.length > 0) {
    router.replace({ name: 'telescope', params: { jid: telescopeModules.value[0]!.jid } })
  }
})
...
// One page ever shows a single module's commands at a time, and every
...
// Reset every command's param values when the module changes (including
// going from one telescope module to another via the sidebar), so a stale
// value never lingers into a module that doesn't even have that field.
watch(
  currentModule,
  (mod) => {
...
// ── Tabs: RA/Dec, Alt/Az, Tracking — only shown as tabs when a module
// implements more than one; a module implementing exactly one renders it
// directly with no tab bar.
...
<template>
  <div style="max-width: 800px">
    <h5 class="text-body fw-semibold mb-4">Telescope</h5>

    <div v-if="telescopeModules.length === 0" class="text-muted" style="font-size:0.9rem">
      <i class="bi bi-info-circle me-1"></i>
      No ITelescope modules online.
    </div>

    <div v-else-if="!currentModule" class="text-muted" style="font-size:0.9rem">
      <i class="bi bi-info-circle me-1"></i>
      Telescope module{{ routeJid ? ` "${routeJid}"` : '' }} is not online.
    </div>
    ...
    <div v-else class="d-flex flex-column gap-2">
      ...
      <div v-if="applicableSections.length > 0" class="rounded-3 p-3 pyobs-panel">
