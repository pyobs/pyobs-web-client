...
build-time codegen against the Python interfaces. See `specs/design/index.md`
and `specs/plans/index.md` for current implementation status.
...
steering — genuinely live there instead, tagged `Repos:`). This repo has no
root-level status doc (matching `pyobs-core`'s own layout): `specs/design/index.md`
is the completed-feature catalog, `specs/plans/index.md` tracks what's still open.
All design docs, implementation plans, and ADRs belong in `specs/`, inline nowhere
else.
